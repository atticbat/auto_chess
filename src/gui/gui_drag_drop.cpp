...
void gui_drag_drop::generate_static_sprite(int id, int frames,
                                           mINI::INIFile file) {
...
gui_base *find_gui_by_id(std::multimap<gui_type, gui_base *> *gui, int id,
                         gui_type state);
static int check_collision(Vector2 mouse_point,
  ...
  for (std::multimap<gui_type, gui_base *>::iterator i = gui->begin();
    ...
    if (i->first == G_DRAG_DROP) {
      gui_drag_drop *drop = dynamic_cast<gui_drag_drop *>(i->second);

      if (drop && CheckCollisionPointRec(mouse_point, drop->bounds) &&
          id != except && drop->display)
...
static void swap_xp_bars(std::multimap<gui_type, gui_base *> *gui,
                         gui_drag_drop *drag, gui_drag_drop *drop,
                         default_run *user) {
  int swap = 0;

  if (drop && drop->display) {
    gui_progress_bar *drag_bar = dynamic_cast<gui_progress_bar *>(
        find_gui_by_id(gui, drag->unique_id + 8, G_PROGRESS_BAR));
    gui_progress_bar *drop_bar = dynamic_cast<gui_progress_bar *>(
    ...
    if (drag_bar && drop_bar) {
      ...
      user->set_unit_exp(drag->gui_id - 5, user->get_unit_xp(drop->gui_id - 5));
...
static void drag_drop_controls(std::multimap<gui_type, gui_base *> *gui,
  ...
  if (drag && IsMouseButtonReleased(MOUSE_LEFT_BUTTON) && drag->is_picked) {
    ...
    if (overlapped > -1) {
      if (!(drag->display)) {
        user->deduct_gold(
            check_price(handle_drop(gui, drag, overlapped, user)));
    ...
    // sold
    if (drag->display &&
        CheckCollisionPointRec(mouse_point,
...
void draw_drag_drops(gui_base *gui, game_settings settings) {
  ...
  if (drag_drop && drag_drop->is_picked)
    drag_drop->draw_sprite(settings.mouse_point, settings.sprite_size, true);
  else if (drag_drop && drag_drop->display && drag_drop->get_sprite_id())
    drag_drop->draw_sprite((Vector2){0, 0}, settings.sprite_size, false);
}
