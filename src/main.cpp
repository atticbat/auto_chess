...
void draw(std::multimap<gui_type, gui_base *> *gui, game_settings settings,
  ...
  case APPLY:
  case SETTINGS: {
    DrawRectangle((settings.screen_dim.x - settings.settings_dim.x) / 2,
                  (settings.screen_dim.y - settings.settings_dim.y) / 2,
                  settings.settings_dim.x, settings.settings_dim.y, GRAY);
  } break;
  case REROLL:
  case DRAFT: {
    DrawRectangle(0, settings.screen_dim.y * 0.75, settings.screen_dim.x,
                  settings.screen_dim.y * 0.25, BROWN);
  } break;
  case SIMULATION: {
    draw_simulation(particles, sprites, settings);
    if (!(settings.user->get_ongoing_game()))
      DrawRectangle(0, 0, settings.screen_dim.x, settings.screen_dim.y,
                    Color{0, 0, 0, 125});
  } break;
  case EDIT_UNIT:
  case DATABASE: {
    DrawRectangle(
        0, 0, settings.screen_dim.x,
        64 * (((DATABASE_INPUTS)*160) / (settings.screen_dim.x - 160)) + 96,
        LIME);
  ...
  if (settings.state == DATABASE)
    draw_grid(check_scrollbar_x(find_gui_by_id(gui, 15, G_SCROLLBAR)),
              check_scrollbar_y(find_gui_by_id(gui, 15, G_SCROLLBAR)), unit_db);
  EndDrawing();
}
...
int main(void) {
  ...
  while (!settings.exit_window) {
    ...
    case SETTINGS: {
      unit_db = settings_controls(&gui, &sprites, &settings, unit_db);
    } break;
    case LOAD: {
    ...
    case SIMULATION: {
      unit_db =
          simulation_controls(&gui, &sprites, &settings, &particles, unit_db);
    } break;
    case DATABASE: {
