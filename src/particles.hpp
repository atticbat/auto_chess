...
typedef enum particle_type { P_BASE = 0, P_DAMAGE, P_PROJECTILE } particle_type;
...
public:
  particle_damage(Rectangle _bounds, Color _colour, int damage, int x_offset)
      : particle(_bounds, _colour, x_offset) {
    char buffer[12];
...
public:
  particle_projectile(Color _colour, unit_stats atk, unit_stats tgt,
      ...
      : particle(atk.get_bounds(), _colour, settings.x_offset) {
    ...
    velocity = abs(tgt.get_bounds().x - atk.get_bounds().x) /
                   ((256 * settings.sprite_size) / 10) -
               0.5 * ((256 * settings.sprite_size) / 6) * grav;
    theta = 0.5 * asin(grav * abs(tgt.get_bounds().x - atk.get_bounds().x) /
                       (velocity * velocity));
  }
  void update_bounds(int x_offset) { original_offset = x_offset; }
  void decrement(void) {
    bounds.x = attacker.x + ((velocity * time * cos(theta)) * 0.5) * direction +
               original_offset;
    bounds.y = attacker.y +
               (velocity * time * sin(theta) * 2 - 0.5 * grav * time * time);
    time += 1;
