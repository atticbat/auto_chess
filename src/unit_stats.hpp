...
public:
  ...
  void initialise_unit(int id, char **unit_db, bool ally, int pos,
    ...
    _unique_id++;
    if (allied) {
      bounds = (Rectangle){(float)(16 + pos) * 256 * scale, 400, 256 * scale,
                           512 * scale};
      direction = 1;
    } else {
      bounds = (Rectangle){(float)(40 - pos) * 256 * scale, 400, 256 * scale,
                           512 * scale};
      direction = -1;
    }
    attack = atoi(split[5]);
  ...
  Rectangle get_gauge_bounds(int index) { return (gauge_bounds[index]); }
  Color get_gauge_colour(int index) { return (gauge_colours[index]); }
  ...
  void set_gauge_bounds(float scale) {
    for (int i = 0; i < 3; i++) {
      gauge_bounds[i] =
          (Rectangle){bounds.x, bounds.y + 512 * scale + (i * 16),
                      (gauges[i] / max_gauges[i]) * (248 * scale), 16};
    }
  }
  ...
  void set_projectile_mid_flight(bool flying) {
    projectile_mid_flight = flying;
  }
