...
void set_database_boundaries(std::multimap<gui_type, gui_base *> *gui,
  ...
  const float header_offset =
      64 * (((DATABASE_INPUTS)*160) / (screen_dim.x - 160)) + 96;
  int index;
  ...
  for (std::multimap<gui_type, gui_base *>::iterator i = gui->begin();
    ...
    if (i->first == G_LABEL) {
      ...
      int remainder =
          64 * (((index - 15) * 160) / ((int)screen_dim.x - 160)) + 32;
      i->second->set_bounds(divided, remainder, (int)i->first, file, 1);
      index++;
    }
  }
...
static void write_unit_db(std::multimap<gui_type, gui_base *> *gui,
  ...
  {
    gui_textbox *textbox =
        dynamic_cast<gui_textbox *>(find_gui_by_id(gui, 16, G_TEXTBOX));
    if (textbox)
  ...
  while (j < DATABASE_INPUTS) {
    fprintf(read_txt, "%s",
            find_gui_by_id(gui, 16 + j, G_TEXTBOX)->text.c_str());
    if (j < 12)
      fprintf(read_txt, "%s", ",");
    j++;
...
void draw_grid(int scroll_h_offset, int scroll_v_offset, char **unit_db) {
  ...
  while (i < MAX_UNITS) {
    ...
    l = 0;
    ft_itoa(i, int_str);
    DrawText(ini.get("UnitNames").get(int_str).c_str(),
             32 + 112 * j + scroll_h_offset, 192 + i * 20 + scroll_v_offset, 16,
             BLACK);
    while (j < 13) {
      if (unit_db[i][k] == ',' || unit_db[i][k] == '\0') {
        ...
        DrawText(ini.get("DatabaseColumns").get(int_str2).c_str(),
                 256 + 112 * j + scroll_h_offset,
                 192 + i * 20 + scroll_v_offset, 16, BLACK);
        j++;
      } else if (unit_db[i][k] >= '0' && unit_db[i][k] <= '9') {
        DrawTextCodepoint(
            GetFontDefault(), unit_db[i][k],
            (Vector2){(float)(328 + 112 * j + 9 * l + scroll_h_offset),
                      (float)(192 + i * 20) + scroll_v_offset},
            16, BLACK);
        l++;
      }
      k++;
